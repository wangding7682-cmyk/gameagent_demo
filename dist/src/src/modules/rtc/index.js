...
function reportRtcDebugEvent(hypothesisId, msg, data = {}, location = 'src/modules/rtc/index.js') {
    ...
    fetch('http://127.0.0.1:7777/event', {
        ...
        body: JSON.stringify({
            sessionId: 'rtc-subtitle-no-text',
            runId: 'pre-fix',
            hypothesisId,
            location,
            msg: `[DEBUG] ${msg}`,
...
function toDebugHexPreview(bytes, limit = 16) {
...
function getDebugObjectKeys(value) {
...
const apiService = {
    ...
    async startVoiceChat(runtimeConfig, session, overrides = {}) {
        ...
        return postJson(`${runtimeConfig.apiBaseUrl}/api/rtc/voice-chat/start`, {
...
export class RtcModule {
    ...
    init() {
        ...
        const enterRtcMode = (config) => {
            ...
            this.eventBus.emit('ABILITY_FEEDBACK', {
                source: 'ui',
                ability: variant === 'screen' ? '桌宠屏幕共享模式' : '桌宠语音模式',
                text: variant === 'screen'
                    ? '开始建立桌宠屏幕共享会话，等待 RTC 连接并接入视频工作区。'
                    : '开始建立桌宠语音会话，等待 RTC 连接和智能体响应。'
            });
    ...
    unpackSubtitleBinaryMessage(message) {
        ...
        // #region debug-point D:subtitle-bytes
        reportRtcDebugEvent('D', 'subtitle-bytes', {
            byteLength: bytes?.length || 0,
            hexPreview: toDebugHexPreview(bytes),
            hasBytes: Boolean(bytes)
        }, 'src/modules/rtc/index.js:454');
        // #endregion
        ...
        // #region debug-point D:subtitle-header
        reportRtcDebugEvent('D', 'subtitle-header', {
            magic,
            byteLength: bytes.length,
            hexPreview: toDebugHexPreview(bytes, 24)
        }, 'src/modules/rtc/index.js:464');
        // #endregion
        if (magic !== 'subv') {
            return null;
        }
        ...
        // #region debug-point D:subtitle-payload
        reportRtcDebugEvent('D', 'subtitle-payload', {
            payloadLength,
            payloadEnd,
            payloadPreview: new TextDecoder().decode(payloadBytes.slice(0, 240))
        }, 'src/modules/rtc/index.js:482');
        // #endregion
        return {
            magic,
            payloadLength,
    ...
    decodeSubtitleMessage(message) {
        ...
        try {
            ...
            // #region debug-point A:subtitle-json
            reportRtcDebugEvent('A', 'subtitle-json', {
                rawType: typeof rawMessage,
                rootIsArray: Array.isArray(parsed),
                rootKeys: getDebugObjectKeys(parsed),
                dataIsArray: Array.isArray(parsed?.data),
                dataLength: Array.isArray(parsed?.data) ? parsed.data.length : 0,
                firstItemKeys: Array.isArray(parsed?.data) && parsed.data[0] ? getDebugObjectKeys(parsed.data[0]) : []
            }, 'src/modules/rtc/index.js:509');
            // #endregion
            ...
            // #region debug-point E:subtitle-json-parse-failed
            reportRtcDebugEvent('E', 'subtitle-json-parse-failed', {
                rawType: typeof rawMessage,
                rawPreview: String(rawMessage || '').slice(0, 240),
                error: error?.message || String(error)
            }, 'src/modules/rtc/index.js:519');
            // #endregion
    ...
    extractSubtitleText(payload = {}) {
        ...
        if (subtitleItems.length > 0) {
            const merged = subtitleItems
                .map((item) => normalizeText(item?.text || item?.message || item?.content))
                .filter(Boolean)
                .join(' ');
            if (merged) {
                return merged;
    ...
    handleRtcSubtitleSignal(uid, streamId, message, source = 'stream-message') {
        ...
        if (!text) {
            // #region debug-point B:subtitle-no-text
            reportRtcDebugEvent('B', 'subtitle-no-text', {
                source,
                uid,
                streamId,
                messageType,
                speakerId,
                rootKeys: getDebugObjectKeys(subtitle),
                dataIsArray: Array.isArray(subtitle?.data),
                dataLength: Array.isArray(subtitle?.data) ? subtitle.data.length : 0,
                firstItem: Array.isArray(subtitle?.data) && subtitle.data[0]
                    ? JSON.stringify(subtitle.data[0]).slice(0, 240)
                    : '',
                rawPreview: JSON.stringify(subtitle).slice(0, 500)
            }, 'src/modules/rtc/index.js:734');
            // #endregion
            console.warn('[RtcModule] 收到字幕信令但未解析出文本', {
                uid,
                streamId,
                source,
                messageType,
    ...
    async attachWorkspaceVideoToScreenShare(reason = 'unknown', payload = {}) {
        ...
        if (!this.isVoiceChatReady) {
            this.emitScreenSharePending('语音聊天尚未就绪，StartVoiceChat 成功后会自动开始共享', {
                reason,
    ...
    async startVoiceChatAfterRtcConnected() {
        ...
        try {
            const runtime = this.refreshRuntimeConfig();
            await apiService.startVoiceChat(runtime, session, startOptions);
    ...
    _bindRtcEvents() {
        ...
        this.rtcEngine.on('onStreamMessageReceived', (uid, streamId, message) => {
            // #region debug-point C:onStreamMessageReceived
            reportRtcDebugEvent('C', 'onStreamMessageReceived', {
                uid,
                streamId,
                messageCtor: message?.constructor?.name || typeof message
            }, 'src/modules/rtc/index.js:1191');
            // #endregion
        ...
        this.rtcEngine.on('onRoomBinaryMessageReceived', (uidOrEvent, maybeMessage) => {
            ...
            // #region debug-point C:onRoomBinaryMessageReceived
            reportRtcDebugEvent('C', 'onRoomBinaryMessageReceived', {
                uid: normalized.uid,
                messageCtor: normalized.message?.constructor?.name || typeof normalized.message,
                rawEventKeys: getDebugObjectKeys(normalized.rawEvent)
            }, 'src/modules/rtc/index.js:1200');
            // #endregion
