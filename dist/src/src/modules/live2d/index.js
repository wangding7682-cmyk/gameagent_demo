...
export class Live2dModule {
    ...
    bindEvents() {
        ...
        const rtcCommandLog = document.getElementById('rtc-command-log');
        ...
        const appendRtcCommandLog = (text, type = 'system') => {
            if (!rtcCommandLog) {
                return;
            }
            const item = document.createElement('div');
            item.className = `rtc-command-item ${type}`;
            item.textContent = text;
            rtcCommandLog.appendChild(item);
            rtcCommandLog.scrollTop = rtcCommandLog.scrollHeight;
        };
        ...
        const appendSessionMessage = (payload = {}) => {
            ...
            if (targets.includes('rtc')) {
                const rtcType = role === 'user'
                    ? 'user'
                    : role === 'error'
                        ? 'error'
                        : 'system';
                appendRtcCommandLog(text, rtcType);
            }
        };
        ...
        rtcInterruptButton?.addEventListener('click', () => {
            appendRtcCommandLog('已请求打断当前播报', 'system');
        ...
        this.eventBus.on('RTC_AGENT_READY', (payload) => {
            appendRtcCommandLog(`智能体已加入房间 ${payload.roomId}，任务 ${payload.taskId} 已启动。`, 'system');
        });
        ...
        this.eventBus.on('RTC_COMMAND_SENT', (payload) => {
            ...
            if (actionLabel === 'ExternalTextToLLM') {
                appendRtcCommandLog(`文本指令已发送：${payload?.message || ''}`, 'system');
                return;
            }
            appendRtcCommandLog(`${actionLabel} 已发送成功`, 'system');
        });

        this.eventBus.on('RTC_COMMAND_FAILED', (payload) => {
            const actionLabel = payload?.action || 'RTC 指令';
            if (actionLabel === 'ExternalTextToLLM') {
                appendRtcCommandLog(`文本指令发送失败：${payload?.error || payload?.message || '未知错误'}`, 'error');
                return;
            }
            appendRtcCommandLog(`${actionLabel} 发送失败：${payload?.error || payload?.message || '未知错误'}`, 'error');
        });
    ...
    configureRtcWindow(variant = 'voice') {
        ...
        const fullTitle = rtcVariant === 'screen' ? '🖥️ 桌宠屏幕共享' : '桌宠语音聊天';
        const compactTitle = rtcVariant === 'screen' ? '屏幕共享' : '桌宠语音聊天';
    ...
    updateRtcVoiceStatus(status = 'connecting') {
        ...
        const titleText = isScreenVariant ? '屏幕共享会话中' : '正在通话中';
        const placeholderText = isScreenVariant ? '正在连接并准备共享视频工作区...' : '正在通话中';
    ...
    syncModalTitle(key, element) {
        ...
        const fullTitle = element.dataset.fullTitle || '正在通话中';
        const compactTitle = element.dataset.compactTitle || '桌宠语音聊天';
        this.videoWindowTitle.textContent = this.modalWindowStates.video.minimized ? compactTitle : fullTitle;
    }
